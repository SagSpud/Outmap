...
// 1. 全国 34 省级行政区中心、地理外包围盒与三维视点 (剥离至独立模块 geo-constants.js)
const PROVINCES_DATA = window.PROVINCES_DATA || {};
...
// 2. 重点地标城市与著名乡镇 (剥离至独立模块 geo-constants.js)
const MAJOR_CITIES = window.MAJOR_CITIES || [];
...
// 路线也需要删除墓碑：仅从当前设备数组移除还不够，下一次双向同步时云端旧路线会被并集合并回来。
const DELETED_ROUTES_STORAGE_KEY = 'outmap_deleted_routes';

function getDeletedRoutes() {
  try {
    const raw = localStorage.getItem(DELETED_ROUTES_STORAGE_KEY);
...
function routeTombstoneKey(item) {
  if (!item || typeof item !== 'object') return '';
  if (item.id) return `id:${item.id}`;
  const start = Array.isArray(item.start) ? item.start.join(',') : '';
  const end = Array.isArray(item.end) ? item.end.join(',') : '';
  const dist = Number.isFinite(Number(item.distKm)) ? Number(item.distKm).toFixed(2) : '';
  return `route:${String(item.name || '').trim()}|${dist}|${start}|${end}`;
}
...
function addDeletedRouteTombstone(route) {
  ...
  try {
    ...
    console.warn('[Outmap] route deletion tombstone could not be persisted:', e);
    return false;
  }
}

function isRouteDeleted(route, deletedList = []) {
...
async function initApplication() {
  ...
  let tileProtocolSupported = false;
  if (window.electronAPI) {
    try {
      ...
      if (info) {
        ...
        if (info.tileProtocol === true) {
          tileProtocolSupported = true;
        }
      }
    } catch (e) {}
  ...
  // 瓦片 API 体系：在 Electron 下若主进程支持 outmap-tile:// 则优先使用特权直通协议（消除 Chromium 6 并发限制与 HTTP Socket 开销）；否则安全使用本地 HTTP 端口
  const canUseTileProtocol = tileProtocolSupported && !isWebMode;
  let demUrl = canUseTileProtocol ? `outmap-tile://dem/{z}/{x}/{y}.webp` : `http://127.0.0.1:${port}/dem/{z}/{x}/{y}.webp`;
  let vecUrl = canUseTileProtocol ? `outmap-tile://vector/{z}/{x}/{y}.pbf` : `http://127.0.0.1:${port}/vector/{z}/{x}/{y}.pbf`;
  ...
  function computeAdaptiveTileCache({ isWeb, isConstrained, dpr, width, height } = {}) {
    ...
    // 桌面端：在 160 到 480 之间平滑动态伸缩，既保障 3D 快速漫游流畅，又彻底杜绝冲破 Chromium SharedImage 显存上限导致 Context Lost
...
function setupPyramidModal(map) {
  ...
  const btnStorageMaintenance = document.getElementById('btn-storage-maintenance');
  ...
  btnStorageMaintenance?.addEventListener('click', (e) => {
    if (e) e.stopPropagation();
    window.openStorageMaintenanceModal?.();
  });
  ...
  if (window.electronAPI && window.electronAPI.onDownloadProgress) {
    window.electronAPI.onDownloadProgress(data => {
      ...
      const isAutoHealing = Boolean(data.autoHealing);
...
async function triggerRealtimeCloudSync(reason = 'change', immediate = false) {
  ...
  const runSync = async () => {
    ...
    try {
      ...
      // 路线删除同样采用墓碑合并，阻止云端旧快照把本地已删路线重新带回。
      const cloudDeletedRoutes = cloudData?.deletedRoutes || [];
      const localDeletedRoutes = getDeletedRoutes();
...
function setupStatusBar(map) {
  ...
  // Windows S0 Modern Standby / lock-screen integration. Chromium's native
  // background throttling handles the hidden renderer. Do not call map.stop()
  // here: stopping a native flyTo would strand the camera between its source
  ...
  if (window.electronAPI && window.electronAPI.onPowerStateChange) {
    ...
    if (typeof removePowerStateListener === 'function') {
      window.addEventListener('beforeunload', removePowerStateListener, { once: true });
    }
  }
}
...
function updateRouteEditUIState(routeOrNull) {
  ...
  const route = routeOrNull || (currentEditingSavedRouteId
  ...
  if (route && currentEditingSavedRouteId) {
    ...
    if (btnSaveRouteTrigger) {
      btnSaveRouteTrigger.innerText = '收藏';
      btnSaveRouteTrigger.classList.remove('is-editing');
      btnSaveRouteTrigger.classList.remove('saved-success');
      btnSaveRouteTrigger.title = '收藏当前规划路线';
    }
    if (btnSaveAsNewRoute) btnSaveAsNewRoute.style.display = 'none';
...
function setupOutdoorRouteSystem(map) {
  ...
  btnSaveRouteTrigger?.addEventListener('click', () => {
    ...
    updateRouteEditUIState(editingRoute);
  ...
  // 确认保存路线到收藏夹（若为编辑态则覆盖更新原路线，若为新建态则新增路线）
  btnConfirmSaveRoute?.addEventListener('click', () => {
    ...
    if (isUpdate) {
      showToast(`✅ 已成功更新路线“${routeName}”！`);
    } else {
      showToast(`✅ 已成功收藏路线“${routeName}”！`);
    }
  });

  // 编辑已有路线时，点击【另存为新路线】(保留原路线不动，生成全新的独立收藏路线)
  btnSaveAsNewRoute?.addEventListener('click', () => {
    const routeName = (saveRouteNameInput?.value || '').trim() || '新建规划路线';
    const effectiveEndCoord = routeEndCoord || (routeViaPoints.length > 0 ? routeViaPoints[routeViaPoints.length - 1].coords : null);
    const effectiveEndName = routeEndName || (routeViaPoints.length > 0 ? routeViaPoints[routeViaPoints.length - 1].name : '终点');
    const effectiveViaPoints = routeEndCoord ? routeViaPoints : routeViaPoints.slice(0, -1);
    const previousRoutes = savedRoutes;
    ...
    try {
      ...
      const reason = /quota|storage|exceed/i.test(rawReason)
        ? '本地存储空间不足'
        : (rawReason ? (rawReason.length > 120 ? `${rawReason.slice(0, 117)}…` : rawReason) : '未知错误');
      alert(`路线另存为失败：${reason}`);
      return;
    }

    savedRoutes = nextRoutes;
    currentEditingSavedRouteId = newRoute.id;
    closeSaveModal();
  ...
  if (canvas) {
    ...
    const handleProfileHover = (clientX) => {
      ...
      // 点 9 优化：高程剖面图视点动态联动
      // 鼠标滑过高程图时，相机中心轻柔对齐对应地形地貌，便于直观感知高山垭口与峡谷
      if (pt.coord && !map.isMoving() && (!map.__outmapGestureActive)) {
        map.easeTo({
          center: pt.coord,
          duration: 120,
          essential: false
