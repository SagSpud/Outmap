...
app.whenReady().then(async () => {
  ...
  try {
    ...
    const result = await win.webContents.executeJavaScript(`(async () => {
      ...
      // 5. 验证 showFluentPrompt DOM 结构与多重提交机制 (pointerdown, touchend, form submit)
      ...
      check(form.tagName === 'FORM', 'Prompt form must be a native FORM tag');
      ...
      // 6. 验证 form submit 提交
      let formSubmitVal = null;
      showFluentPrompt({
        title: '测试回车提交',
        initialValue: '',
        onConfirm: (val) => { formSubmitVal = val; }
      });
      const overlay2 = document.querySelector('.fluent-prompt-overlay');
      const input2 = overlay2.querySelector('.fluent-prompt-input');
      input2.value = '通过FormSubmit提交';
      const form2 = overlay2.querySelector('.fluent-prompt-form');
      form2.dispatchEvent(new Event('submit', { bubbles: true, cancelable: true }));
      await sleep(100);
      check(formSubmitVal === '通过FormSubmit提交', 'form submit failed: ' + formSubmitVal);
      ...
      return {
        ...
        formSubmitPassed: true,
        moreButtonsPresent: true
      };
    })()`);

    console.log('v1.9.40 mobile interaction, prompt form, and cloud sync LWW regression passed:', result);
    clearTimeout(watchdog);
    win.destroy();
    app.exit(0);
  } catch (err) {
    console.error('v1.9.40 test failed:', err);
    clearTimeout(watchdog);
    if (win) win.destroy();
    app.exit(1);
  }
