...
const workerSrc = fs.existsSync('src/offline-worker.cjs') ? fs.readFileSync('src/offline-worker.cjs', 'utf8') : '';

// main.js / offline-worker.cjs 检查: 离线三态阈值与防卡死
assert(mainSrc.includes('Math.floor(expected * 0.96)') || workerSrc.includes('level.expected'), 'must verify expected offline level');
assert(mainSrc.includes('partialZ = Math.max(partialZ, z)') || workerSrc.includes('partialZ'), 'must record partial levels');
assert(mainSrc.includes('inventoryVersion') || mainSrc.includes('loadOfflineManifest') || mainSrc.includes('hasProvRecord'), 'main.js getQuickTileCount must avoid full disk scan when manifest is present');
