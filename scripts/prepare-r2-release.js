...
async function main() {
  ...
  const defaultNotes = [
    '1. 修复长距离飞往高海拔收藏点时偶发只剩地图底色、必须缩放或拖动才能恢复的问题；',
    '2. 地形未加载时不再把收藏记录中的海拔提前写入 MapLibre 相机，避免相机进入无有效瓦片覆盖的高度状态；',
    '3. 真实 DEM 到达后仍由 MapLibre 原生地形采样完成落地校正，保留 3D 视角与居中偏下位置；',
    '4. 保留 2.0.2 的触屏菜单和道路编号修复，不修改现有离线地图、收藏、路线、画质与动画。'
  ].join('\n');
