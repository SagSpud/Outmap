...
async function main() {
  ...
  const defaultNotes = [
    '1. 【顶栏Logo翻转层叠彻底修复】强化品牌Logo 3D翻转卡片正反面绝对定位与 backface-visibility: hidden 物理隔离，彻底根治版本号角标横向并排外露与垂直倒置问题；',
    '2. 【弹窗防透色实体白底】存储体检与各级模态浮层彻底采用 100% 实体纯白背景与深色高对比度排版，深色毛玻璃遮罩强力降暗底层，彻底杜绝文字穿透重影；',
    '3. 【按键调度层级严格隔离】全局键盘分发器在捕获阶段最高优先级截获 Escape 键，严格遵循“子浮层先关、父窗口后关”的拓扑层级，彻底修复底层离线下载窗口意外先退出的问题；',
    '4. 【全场景自适应丝滑飞掠】基于对数距离模型和 C2 连续曲线重塑相机飞掠动力学，自动根据当前缩放与跨场景尺度动态计算时长与俯仰，彻底消除不同缩放比例下飞掠突变与眩晕卡顿；',
    '5. 【离线下载看门狗与架构优化】集成 Stall Watchdog 与 7500ms 硬超时沙箱自愈机制，保持 9 项深度高能架构与 GPU 显存稳定性；',
