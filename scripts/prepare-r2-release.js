...
async function main() {
  ...
  const defaultNotes = [
    '1. 修复收藏夹首次远距离飞掠的二次拉回与落点漂移，2D、50° 和 70° 视角统一使用 MapLibre 原生相机投影；',
    '2. 修复路线搜索重复飞行，强化超长路线、多途径点、三种模式隔离、请求取消与缓存上限；',
    '3. 修复离线省份、层级和图层计数误判，下载失败或中止后可继续补齐，不影响现有离线地图；',
    '4. 优化启动、云同步、2D/3D 切换和地图移动性能，保留毛玻璃与动画并减少重复合成；',
    '5. 导航路线更新为清晰蓝色且不遮挡道路名称，指针改用 Windows/macOS 原生样式。'
  ].join('\n');
