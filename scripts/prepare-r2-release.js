...
async function main() {
  ...
  const defaultNotes = [
    '1. 收藏点右键菜单移除标题区，保留类型选择并新增底部删除操作；',
    '2. 收藏路线改为 MapLibre 原生 GeoJSON 路线图层，启动后默认显示并支持点击载入；',
    '3. 图层面板明确提供收藏地点与收藏/规划路线开关，分别控制对应原生图层；',
    '4. 统一收藏点与途径点聚合数字的尺寸、字重和视觉比例；',
    '5. 路线收藏成功后静默完成，只有真实保存失败时显示“路线保存失败：原因”；',
    '6. 路线删除增加跨端同步墓碑，避免云端旧快照导致已删除路线立即复活；',
    '7. 收藏点与收藏路线统一使用 Fluent 右键菜单；路线菜单改为内容自适应宽度并增加导出/删除前置图标；收藏点列表移除多余删除图标；\n8. 修复收藏点右键菜单底部操作被裁切，并缓存更新检查/下载任务，切换界面不会重复检查或重复下载；\n9. 收藏点类型菜单按内容收窄，选中标记不再撑开右侧空白，删除操作固定可见。'
  ].join('\n');
