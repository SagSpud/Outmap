...
async function main() {
  ...
  const defaultNotes = [
    '1. 【路线点点击交互治理】彻底阻断点击路线点时的底层事件向下穿透，杜绝点击路线点或路线时自动插入重复途径点并强行跳入编辑界面的侵入行为；',
    '2. 【新增路线点位原生 Fluent 查看卡片】单击起点、途径点、终点时展示点位角色徽章、名称、所属路线、沿途累计里程、实时海拔高程与经纬度，并提供视角聚焦与快捷收藏；',
    '3. 【线上海拔点实时探针与自主途经点设置】单击路线轨迹任意位置时，动态吸附并解算距起点里程、剩余里程与 DEM 精确海拔；仅在用户主动点击【设为途经点】时才加入规划；',
    '4. 【全生命周期联动与状态隔离】路线清空、还原或切换状态时自动联动关闭查看卡片，3D 地图平滑漫游时保持动态投影跟随。'
  ].join('\n');
