...
async function main() {
  ...
  const defaultNotes = [
    '1. 【陡峭 3D 缩放稳定】恢复官方 MapLibre 6.9 相机与地形碰撞链，缩放固定为屏幕中心锚点，解决手势结束后突然变大、反向回弹和指针位置引起的视图偏移；',
    '2. 【飞掠与画质保持】继续保留目标地形预热、居中偏下落点、3D 地形、等高线、毛玻璃与动画效果，不降低渲染画质；',
    '3. 【静止占用与图层清理】移除常驻逐帧性能轮询、私有 RTT 强制释放以及永远无数据的路线聚合图层，减少静止唤醒并避免飞掠后偶发空白；',
    '4. 【离线增量更稳】PMTiles 在旧归档持续可读时分批构建，最终短暂原子替换并支持失败回滚，减少下载组装期间的地图卡顿或空白。'
  ].join('\n');
