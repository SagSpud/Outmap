...
async function main() {
  ...
  const defaultNotes = [
    '1. 【离线下载卡顿彻底解决】修复缺片定位与并发下载通道协作机制，流式发现缺片即可全速下载，杜绝定位中零速度假死；',
    '2. 【磁盘 I/O 极速优化】优化散列瓦片与 PMTiles 缺片检测，彻底消除冗余 readdir 系统调用开销；',
    '3. 【下载状态实时反馈】已就绪瓦片与下载进度实时联动更新，精准显示已补齐瓦片数、真实网络吞吐与下载百分比；',
    '4. 【界面精炼无冗余】精简界面状态显示，去除不必要的冗余弹窗与文案。'
  ].join('\n');
