...
async function main() {
  ...
  const defaultNotes = [
    '1. 修复飞掠残留目标和复杂地形防穿模校正导致的滚轮放大后回缩问题；',
    '2. 为滚轮、触控缩放、键盘和双击缩放增加方向保护，达到安全视距时平滑停住而非反向跳动；',
    '3. 覆盖 L3.9 至 L16.8、0°/50°/70°及地图标记与覆盖层交互，不降低画质、3D、动画和毛玻璃；',
    '4. 保留 2.0.6 路线重命名 R2 源站原子写入与跨端同步修复。'
  ].join('\n');
