...
async function main() {
  ...
  const defaultNotes = [
    '1. 【下载图层免选与全量默认】移除冗余的图层复选框行，默认全量下载 DEM 高程图与 OSM 矢量路网建筑，操作直观省心；',
    '2. 【底部操作栏排版协调统一】将“检查更新”按钮下移至对话框底部左侧，与“存储体检”保持一致规范外观（13px 图标、统一高度与次级按钮材质），与右侧主操作区左右呼应，界面极具美感与平衡感；',
    '3. 【下载任务图层全联动】离线金字塔下载与增量校验引擎原生默认全图层（DEM + 矢量），空间索引与缺片扫描自适应双图层完整性保障。'
  ].join('\n');
