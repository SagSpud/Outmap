...
async function main() {
  ...
  const defaultNotes = [
    '1. 图层面板将蓝色收藏路线与绿色当前规划路线拆成两个独立开关，不再一并显示或一并关闭；',
    '2. 收藏路线默认关闭、规划路线默认开启，启动后不再把同一路线以蓝绿两层重叠显示；',
    '3. 规划路线开关只控制当前路线、导入轨迹及起点/途径点/终点，收藏路线开关只控制已保存路线；',
    '4. 移除与右侧 2D/3D 按钮功能重复的“3D地貌效果”开关，避免两套状态互相冲突；',
    '5. 右侧 2D/3D 切换继续完整控制 DEM 地形、山体阴影和 50 度三维视角，3D 能力没有删减；',
    '6. 收藏夹路线标签统一更名为“收藏路线”；不修改现有路线、收藏或离线地图数据。'
  ].join('\n');
