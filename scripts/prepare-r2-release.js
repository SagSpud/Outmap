...
async function main() {
  ...
  const defaultNotes = [
    '1. 修复路线规划纯途径点自动提升：首个有效途径点自动提升为起点，末个途径点自动提升为终点，底栏输入框与地图图钉实时同步；',
    '2. 统一桌面端与网页端云端漫游同步上传入口，网页端接入 Web Crypto S3 原生签名直传 R2；',
    '3. 优化收藏夹跨设备智能去重（按坐标精细精度与同名匹配），引入跨端删除墓碑机制，彻底根除地标死而复生与多端不一致问题。'
  ].join('\n');
