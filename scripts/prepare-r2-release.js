...
async function main() {
  ...
  const defaultNotes = [
    '1. 修复离线下载卡在 0 KB/s 问题：DEM 地形数据源全球最高仅到 12 级，下载 L13/L14 时严格规整 DEM 范围，不再发起无效 404 请求，下载保持高速顺畅；',
    '2. 优化下载进度条底部信息布局：将“已补齐/无数据/失败”等状态独立移至下一行展示，主行专用于已下载瓦片数与下载速率，不再挤压重叠；',
    '3. 收藏夹文件夹重命名防重复，支持全部分类标签原生拖拽重排与右键/双击重命名及删除；',
    '4. 中止下载与重试时彻底释放锁，避免出现“已有下载或校验正在进行中”报错；',
    '5. 完整保留全部已有离线瓦片与目录配置，不移动、不删除任何本地数据。'
  ].join('\n');
