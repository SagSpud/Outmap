...
async function main() {
  ...
  const defaultNotes = [
    '1. 【底部状态栏交互三合一】底栏支持一键点击复制当前坐标（带轻量反馈）、点击航向正北平滑归正、点击俯仰一键切换 2D 垂直/3D 透视视角；',
    '2. 【地图右键副标题唤醒】右键菜单副标题实时显示所击位置的高程海拔与精准经纬度，无需移开视线即可洞察地貌特征；',
    '3. 【路线规划顶栏激活 GPX 导出】在面板顶栏显式加入「导出」按钮，规划后一键导出标准 GPX 1.1 文件；起终点对调图标升级为直观双向箭头；',
    '4. 【搜索与全屏沉浸野外模式】新增 Ctrl+F / Cmd+F 快捷键兼容及中文标点经纬度直达；支持 F11 纯净全屏沉浸野外勘察模式（Esc 顺畅退出）；',
    '5. 【文案极简化与磁盘容量感知】去除多余叨扰弹窗与冗长教程文案；离线下载弹窗清晰感知磁盘剩余可用空间。'
  ].join('\n');
