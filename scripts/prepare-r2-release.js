...
async function main() {
  ...
  const defaultNotes = [
    '1. 【根除聚焦画面缩放抖动】针对视野内与 35km 内点位聚焦，彻底停用 MapLibre 原生抛物线拉高绕飞算法（van Wijk flyTo），采用单阶段精准平滑滑行（easeTo），彻底杜绝画面先缩放再放大的拉扯与视界抖动；',
    '2. 【近距聚焦保持用户视级（零缩放）】当用户已放大查看路线点（当前缩放级别 >= 12.0）时，点击聚焦或点位严格维持用户当前缩放层级，不再粗暴强制缩放至 12.0，实现真正的零缩放平滑平移；',
    '3. 【彻底消除镜头落地抽搐与微抖】近距滑行全程交由 MapLibre 原生相机动力学解算，彻底禁用末端 28% 阶段的 setLocationAtPoint 强制反向拉扯与落地生硬瞬移，落地过程 C2 阶导数严格归零；',
    '4. 【聚焦位置绝对居中】路线点聚焦动作统一对齐视觉几何中心（centered: true），彻底消除将目标点压低至屏幕下方 68% 的偏离感。'
  ].join('\n');
