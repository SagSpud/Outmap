...
async function main() {
  ...
  const defaultNotes = [
    '1. 修复密集路线点重叠：原生 MapLibre 图层按屏幕空间稳定散开，支持 2D、50°/70° 3D 与大量途径点；',
    '2. 优化拖动、缩放和飞掠：下载任务在地图交互时自适应让出资源，本地瓦片与字体改为异步读取；',
    '3. 消除空闲 FPS 定时唤醒、多余初始化补帧和下载完成后的重复全国扫描，降低静止 CPU/GPU 占用；',
    '4. 统一桌面、网页与手机端的毛玻璃、弹层和进出动画，移动端自动使用更轻量的视觉效果。'
  ].join('\n');
