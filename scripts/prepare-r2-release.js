...
async function main() {
  ...
  const defaultNotes = [
    '1. 【长时间运行输入自愈】统一收口地图、路线点、途径点排序和收藏夹拖动状态，失焦、最小化、锁屏或休眠时即使系统漏发 pointerup，也不会再留下点不动的指针捕获；',
    '2. 【原生 MapLibre 交互恢复】回到前台后重置仍启用的原生拖动、旋转、滚轮、键盘和触控处理器，清除中断的透明关闭层与临时拖动元素，不改变正常飞掠目标和操作手感；',
    '3. 【零轮询稳定机制】恢复流程完全由窗口与电源事件驱动，不增加常驻计时器或后台进程；继续保留旧离线地图、PMTiles、收藏、路线和同步数据兼容。'
  ].join('\n');
