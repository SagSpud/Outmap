...
async function main() {
  ...
  const defaultNotes = [
    '1. 修复应用启动时收藏路线图层可能未创建，导致打开“收藏路线”后仍然不显示的问题；',
    '2. 收藏路线在 MapLibre 地图加载完成后自动补建原生 source 与 line 图层，并立即载入已有收藏数据；',
    '3. 打开收藏路线开关时会主动校验并刷新图层，不再依赖偶然的后续收藏或同步操作；',
    '4. 收藏路线仍默认关闭，规划路线仍默认开启，两类路线继续保持独立，不修改现有收藏、路线或离线地图数据。'
  ].join('\n');
