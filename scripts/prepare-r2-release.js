...
async function main() {
  ...
  const defaultNotes = [
    '1. 【路线点飞掠流畅化】详情毛玻璃卡片延后到原生飞掠落地后显示，避免复杂 3D 地形飞行期间参与逐帧合成与投影；',
    '2. 【长路线计算缓存】累计里程与精确屏幕投影按路线和相机状态复用，减少超长 GPX 重复查看、插入途径点时的主线程计算；',
    '3. 【历史残留安全精简】清理已停用的 DOM 标记、旧更新弹窗、路线模拟器与重复离线状态写入逻辑，同时完整保留旧离线地图、PMTiles、收藏和云同步兼容。'
  ].join('\n');
