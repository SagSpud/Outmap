...
async function main() {
  ...
  const defaultNotes = [
    '1. 修复滚轮缩放以指针位置为锚点导致的地图横向偏移，改为 MapLibre 原生视口中心缩放；',
    '2. 修复 3D 地形飞掠中 DEM 分级到达导致的抖动、拉回和落点偏移，继续保持目标在屏幕中间偏下；',
    '3. 修复 50°/70° 复杂地形下滚轮缩放方向反转和过度跳级，保留 MapLibre 原生动画与画质；',
    '4. 修复旧收藏路线重读后身份变化、同名路线误合并以及删除后复活问题；',
    '5. 增加搜索缓存过期与收藏变更失效机制，修复同大小离线瓦片更新后可能读取旧缓存的问题；',
