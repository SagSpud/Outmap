...
async function main() {
  ...
  const defaultNotes = [
    '1. 飞掠默认着陆层级全面优化为 12.0 黄金视级：省去近 75% 瞬时深层瓦片与 DEM 网格解算等待，消除视锥反压，着陆视野更开阔更平滑；',
    '2. 地图最大缩放层级上限优化调整为 16（配合聚合阈值 15）：杜绝深层切片过度超采样拉伸马赛克与显存浪费，最大层级下所有点位独立展开；',
    '3. 收藏点悬停提示（Hover Tooltip）：鼠标指针移至收藏点变手的同时浮现 Fluent 亚克力微徽章，优雅展示地点名称、类型图标与高程信息，移出平滑淡出；',
    '4. 彻底根治右键收藏点“地点信息一闪而过”竞态缺陷：精准阻断全局通用菜单误弹，右键收藏点秒开类型修改专属菜单。'
  ].join('\n');
