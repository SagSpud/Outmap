...
async function main() {
  ...
  const defaultNotes = [
    '1. 【路线浏览/编辑模式分离】打开收藏路线默认进入只读浏览模式，防止意外误改；点击“编辑”进入编辑模式，支持保存修改、另存为新路线与一键还原；',
    '2. 【视口智能安全避让】路线规划与加载时动态计算右侧面板宽度，自动配置 fitBounds 安全 Padding，确保整条路线轨迹完整显示不被遮挡；',
    '3. 【收藏夹即时搜索过滤】收藏夹抽屉支持点位与路线即时关键字过滤与一键清除，查找海量户外点位路线更便捷；',
    '4. 【触控端途径点拖拽优化】全面支持触控设备（平板/触屏本）触摸拖动起点、途径点与终点调整路线；',
    '5. 【路线点击快捷加点】路线编辑模式下，直接点击地图上的路线轨迹线即可就近插入新途径点并自动重排计算。'
  ].join('\n');
