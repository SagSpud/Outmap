...
async function main() {
  ...
  const defaultNotes = [
    '1. 【路点顺序与聚合展示优化】彻底关闭路线规划与编辑路点的聚合（cluster: false），所有途经点清晰按顺序显示（起、1、2、3...终），并在标记下方优雅显示对应地名，告别橘黄色重叠数字圈；',
    '2. 【路线保存按钮紧凑美观】保存成功后提示文案精简为“已保存”（去除对勾与长文字），彻底解决按钮栏拥挤溢出问题；',
    '3. 【删除路线防复活机制加固】修复删除墓碑因沿用轨迹历史时间导致云同步时被判定为“删除后修改”而误复活的漏洞，永久防止已删路线再次出现；',
    '4. 【规划路线与收藏路线防重叠】规划路线与收藏路线重叠时自动隐藏对应收藏图层，清空规划即刻恢复；',
