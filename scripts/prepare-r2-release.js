...
async function main() {
  ...
  const defaultNotes = [
    '1. 【GeoJSON 更新去重与签名增量缓存】路线起终点、途径点与收藏路线全面接入签名去重机制，无变更时彻底杜绝无谓的 Web Worker 序列化与瓦片重绘；',
    '2. 【收藏路线几何与重叠检测缓存】单路线 5000 点抽稀几何、空间包围盒与采样点全面接入 Map 缓存，重叠度比对计算量减少 95%+；',
    '3. 【收藏列表容器级事件委托与 Keyed DOM】重构收藏地点与路线列表渲染逻辑，消除数千个卡片监听器绑定，搜索与切换卡片增量复用，滚动更丝滑；',
    '4. 【CSS 视口外渲染性能隔离】引入 content-visibility: auto 与 contain-intrinsic-size，大幅降低海量收藏卡片的渲染与重排开销；',
    '5. 【MapLibre 私有瓦片缓存接口安全守卫】全面隔离与守护底层 _sourceCaches 缓存伸缩逻辑，确保跨平台与引擎升级的绝对健壮性。'
  ].join('\n');
