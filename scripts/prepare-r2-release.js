...
async function main() {
  ...
  const defaultNotes = [
    '1. 【离线下载面板文字重叠与排版治理】重构下载进度卡片内部栅格与弹性盒子布局，彻底解决长任务标题与状态/百分比文字重叠、字符截断截半的问题；',
    '2. 【单行省略号标准支持】将任务名称容器标准化为 block + 弹性伸缩，激活标准 text-overflow: ellipsis 省略号支持，超长省份名称自适应优雅截断不再挤压右侧状态；',
    '3. 【信息列与状态列比例优化】精简左侧静态指标列宽度至 148px，为右侧实时下载状态列释放更多横向展示空间，高分屏下提升卡片宽度比例；',
    '4. 【任务状态文案精简提炼】将定位阶段标题由冗长的“正在定位未下载部分: ...”提炼精简为“定位缺片: ...”，消除与右侧“定位中”徽章文案冗余，界面清爽规整。'
  ].join('\n');
