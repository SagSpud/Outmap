...
async function main() {
  ...
  const defaultNotes = [
    '1. 修复 3D 地形下搜索、收藏和路线定位偶发空白、落地后拉动及中心偏移；',
    '2. 统一为单条 MapLibre 原生相机轨迹，滚轮缩放结束后不再轻微位移或反向回缩；',
    '3. 使用共享 DEM 高程预热与双线性采样，兼顾高山落点稳定、飞掠流畅和资源占用；',
    '4. 优化增量更新状态文字在 200% 高分屏和手机端的完整显示，不改动现有离线地图。'
  ].join('\n');
