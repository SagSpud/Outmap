...
async function main() {
  ...
  const defaultNotes = [
    '1. 【后台节能不再打断飞掠】交由 Chromium 原生后台节流处理隐藏窗口，最小化、锁屏或休眠不再强制终止 MapLibre 相机动画，恢复后仍能准确到达原目标；',
    '2. 【电源生命周期稳定化】Windows S0 待机、锁屏与电池状态统一纳入真实非活动状态判断，监听只注册一次；长时间后台按内存压力温和修剪瓦片缓存，前台画质和帧率不降级；',
    '3. 【恢复完整发布回归门禁】重新启用路线、收藏、离线下载、3D 分数层级、移动端交互和性能等完整测试，并继续保留旧离线地图、PMTiles 与同步数据兼容。'
  ].join('\n');
