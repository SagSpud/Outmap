...
async function main() {
  ...
  const defaultNotes = [
    '1. 【离线文件路径自适应增强】自适应识别多层解压嵌套结构（如 Documents/Outmap/Outmap），离线数据统一存放在规范的 offline-tiles 目录，且各层级 PMTiles 归档文件均能毫秒级直读识别；',
    '2. 【离线目录直达】下载管理界面新增“打开离线目录”按钮及存储位置实时显示，支持在 Windows 资源管理器中一键直通 PMTiles 归档库；',
    '3. 【轻量增量热更新】全面启用应用内热更新（~4MB），零弹窗毫秒级覆盖，告别重复下载全量大安装包；',
    '4. 【清理旧环境残留】彻底净化云端多余测试数据，提供配套清理脚本，全面清理注册表与散列瓦片缓存。'
  ].join('\n');
