...
async function main() {
  ...
  const defaultNotes = [
    '1. 【彻底消除浏览模式误报弹窗】点位按压与触控不再误触发“处于浏览模式”提示，彻底恢复清爽无打扰的地图点位浏览体验；',
    '2. 【浏览模式纯净飞掠聚焦】在浏览模式下点击路线点恢复为纯粹平滑的 60 FPS 满帧飞掠定位，不强制弹出卡片遮挡视线，右键或长按可随时呼出详情；',
    '3. 【彻底根治相机飞掠掉帧卡顿】飞掠启动瞬间立即自动收起并冻结一切浮动点位卡片，杜绝飞行期间带有亚克力毛玻璃高斯模糊（backdrop-filter）的 DOM 逐帧重绘拖垮 GPU 渲染管线，飞掠全程丝滑满帧；',
    '4. 【点位卡片 GPU 硬件加速与 RAF 节流】点位卡片采用 translate3d 硬件加速变换并配合 requestAnimationFrame 单帧节流，杜绝主线程与样式过渡动画冲突。'
  ].join('\n');
