...
async function main() {
  ...
  const defaultNotes = [
    '1. 【离线下载弹窗精简优化】彻底移除预计大小后附带的可用磁盘空间提示，杜绝文字过长导致的横向重叠，界面更紧凑清爽；',
    '2. 【状态栏交互三合一】底部状态栏支持一键点击复制当前坐标（带轻量反馈）、点击航向正北平滑归正、点击俯仰一键切换 2D/3D 透视视角；',
    '3. 【地图右键副标题】右键菜单实时显示所选点的高程海拔与精准经纬度；',
    '4. 【路线规划顶栏激活 GPX 导出】在面板顶栏显式加入「导出」按钮，规划后一键导出标准 GPX 1.1 文件；起终点对调图标升级为直观双向箭头；',
    '5. 【搜索与沉浸野外模式】新增 Ctrl+F / Cmd+F 快捷键及中文标点经纬度直达；支持 F11 纯净全屏沉浸野外勘察模式（Esc 顺畅退出）。'
  ].join('\n');
