...
async function main() {
  ...
  const defaultNotes = [
    '1. 【深度支持 Windows S0 现代待机与后台节电】接入 Electron powerMonitor 电源生命周期，系统合盖睡眠（suspend）或锁屏时自动刹停 3D 相机惯性渲染、修剪瓦片内存，使 CPU/GPU 彻底归零进入深空闲（DRIPS），杜绝背包发烫与待机掉电；',
    '2. 【唤醒与电源感知即刻无缝恢复】系统从 S0 待机唤醒或解除锁屏后单次重绘瞬时恢复，并感知电池/交流电状态自适应调优性能；',
    '3. 【延续 v2.0.73 架构精简成果】保持路线点毛玻璃按需渲染、长路线里程缓存与精简纯净的架构，全流程极速稳定。'
  ].join('\n');
