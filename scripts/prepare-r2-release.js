...
async function main() {
  ...
  const defaultNotes = [
    ...
    '9. 修复合法的小尺寸矢量 PBF 被误判为失败或被本地服务清理的问题；',
    '11. 收藏抽屉文件夹标签支持原生拖拽排序并实时持久化，支持右键重命名、删除及双击重命名；',
    '12. 收藏地点（地图图钉与抽屉列表）及收藏路线右键菜单新增“重命名...”，采用原生 Fluent 极简毛玻璃弹窗；',
    '13. 地点右键菜单中“重命名”置于底部、路线右键菜单中“重命名路线”置于底部；',
    '14. 优化航点飞跃距离耗时连续算法，远距高俯仰飞跃期间实施图层缓冲保护，彻底根除 3D 地形图层瓦片闪烁与拉扯；',
    '15. 保留现有离线目录和全部瓦片，不删除、不移动、不重建已有数据。'
  ].join('\n');
