...
async function main() {
  ...
  const defaultNotes = [
    '1. 路线编辑全流程可视化反馈：载入规划时直观展示【编辑中】横幅与路线名，保存按钮高亮指示“保存修改”，保存成功后短暂显示绿色“✓ 已保存修改”；',
    '2. 新增【另存为新路线】选项：在保存弹窗中一键将修改后的路线另存为全新独立路线，原路线完整保留；',
    '3. 路线规划面板新增【退出编辑】功能，随时解除原路线编辑绑定回到自由规划模式；',
    '4. 收藏抽屉列表中当前正在编辑的路线实时高亮并显示【编辑中】专属标识；',
    '5. 新增独立等高线离线预生成工具（提供 Windows 双击即用 .bat 脚本）：支持指定本地 DEM 目录 100% 离线急速生成 PMTiles 单文件归档，支持智能在线补充缺失切片与海平面平滑边缘垫底。'
  ].join('\n');
