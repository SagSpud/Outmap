...
async function main() {
  ...
  const defaultNotes = [
    '1. 【解除中国区域限制，支持全球自由漫游】彻底移除地图 maxBounds 边界锁死与阻尼回弹，支持平移拖动至中国以外的世界任意大洲大洋与国家地区；',
    '2. 【宏观视野与世界连续环绕】将 minZoom 适配扩展至 2.0 并开启经度连续环绕（renderWorldCopies），支持宏观跨大洲鸟瞰与环球漫游；',
    '3. 【严格锁定默认初始视角】启动应用时严格保持当前中国全境总览默认视图（中心点 [104.5, 36.0]，缩放 4.45），原生体验零偏移；',
    '4. 【境外全图层即看即缓存】彻底移除境外高缩放（z >= 11）DEM 与矢量切片拦截限制，浏览全球任意区域时动态拉取实时高清矢量网、地形 DEM 与影像，并通过防击穿并发队列自动写盘持久化缓存，离线秒开；',
    '5. 【全球搜索与地名解析支持】解除搜索接口硬编码的中国边界过滤，支持搜索境外地点（如“东京”、“富士山”、“阿尔卑斯”等），地名反查智能结合 OSM 渲染切片提取本地地名。'
  ].join('\n');
