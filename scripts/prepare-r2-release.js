...
async function main() {
  ...
  const defaultNotes = [
    '1. 修复收藏路线输入中文名称时，输入法选字 Enter 被误判为确认而导致重命名不生效的问题；',
    '2. 路线重命名改为先持久化完整新数据、成功后再更新界面和云端，避免刷新后恢复旧名称；',
    '3. 收藏路线右键菜单文案由“重命名路线”精简为“重命名”；',
    '4. 保留 2.0.3 的高海拔飞掠空白修复，不修改现有离线地图、路线轨迹和画质。'
  ].join('\n');
