...
async function main() {
  ...
  const defaultNotes = [
    '1. 修复 1.9.1 路线点在不同缩放比例下发生视觉位移的问题：所有点始终严格使用真实经纬度；',
    '2. 近距离路线点改用 MapLibre 原生聚合，低缩放显示数量、放大后恢复独立起点/终点/途径点；',
    '3. 保留 1.9.1 的拖动、缩放、飞掠、下载调度、异步瓦片读取与静止 GPU 优化；',
    '4. 保留桌面、网页与手机端统一的毛玻璃、弹层和进出动画。'
  ].join('\n');
