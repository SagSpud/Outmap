...
process.on('uncaughtException', (err) => {
  console.error('UNCAUGHT EXCEPTION in v1.6.8:', err);
  process.exit(1);
});
...
assert(['1.6.8', '1.6.9', '1.7.0', '1.7.1', '1.7.2', '1.7.3'].includes(packageJson.version), 'package.json version must be valid');
assert(appJs.includes("const APP_VERSION = '1.6.8'") || appJs.includes("const APP_VERSION = '1.6.9'") || appJs.includes("const APP_VERSION = '1.7.0'") || appJs.includes("const APP_VERSION = '1.7.1'") || appJs.includes("const APP_VERSION = '1.7.2'") || appJs.includes("const APP_VERSION = '1.7.3'"), 'app.js must declare APP_VERSION');
assert(indexHtml.includes('style.css?v=1.6.8') || indexHtml.includes('style.css?v=1.6.9') || indexHtml.includes('style.css?v=1.7.0') || indexHtml.includes('style.css?v=1.7.1') || indexHtml.includes('style.css?v=1.7.2') || indexHtml.includes('style.css?v=1.7.3'), 'index.html must reference style.css');
assert(indexHtml.includes('location-camera.js?v=1.6.8') || indexHtml.includes('location-camera.js?v=1.6.9') || indexHtml.includes('location-camera.js?v=1.7.0') || indexHtml.includes('location-camera.js?v=1.7.1') || indexHtml.includes('location-camera.js?v=1.7.2') || indexHtml.includes('location-camera.js?v=1.7.3'), 'index.html must reference location-camera.js');
assert(indexHtml.includes('app.js?v=1.6.8') || indexHtml.includes('app.js?v=1.6.9') || indexHtml.includes('app.js?v=1.7.0') || indexHtml.includes('app.js?v=1.7.1') || indexHtml.includes('app.js?v=1.7.2') || indexHtml.includes('app.js?v=1.7.3'), 'index.html must reference app.js');
assert(indexHtml.includes('v1.6.8') || indexHtml.includes('v1.6.9') || indexHtml.includes('v1.7.0') || indexHtml.includes('v1.7.1') || indexHtml.includes('v1.7.2') || indexHtml.includes('v1.7.3'), 'index.html must display version badge');
...
app.whenReady().then(async () => {
  ...
  const watchdog = setTimeout(() => {
    console.error('v1.6.8 test timed out after 45s');
    app.exit(1);
  }, 45000);
  ...
  assert(['v1.6.8', 'v1.6.9', 'v1.7.0', 'v1.7.1', 'v1.7.2', 'v1.7.3'].includes(results.badgeText), 'Brand badge must display valid version');
